...
const AdminPage = () => {
  ...
  const [category, setCategory] = useState(categoryGroups[0]?.subcategories[0]?.name || 'Chair');
  ...
  const handleAddProduct = async (event: FormEvent<HTMLFormElement>) => {
    ...
    setImageUploadData('');
    setPrice('');
    setCategory(categoryGroups[0]?.subcategories[0]?.name || 'Chair');
    setSoldOut(false);
            ...
            {!isLoggedIn ? (
              ...
              <div className="space-y-8">
                <form onSubmit={handleAddProduct} className="space-y-4 rounded-2xl border border-black/10 bg-[#F5F5F7] p-4 sm:p-5">
                  ...
                  <label className="block">
                    <span className="mb-2 block text-sm font-semibold text-[#1A1A1A]">Category</span>
                    <select
                      value={category}
                      onChange={(e) => setCategory(e.target.value)}
                      className="h-11 min-h-[44px] w-full rounded-xl border border-black/15 px-3 bg-white"
                      required
                    >
                          ...
                          <div className="pt-3">
                            ...
                            <button
                              ...
                              onClick={() => {
                                ...
                                setCategory(item.category || categoryGroups[0]?.subcategories[0]?.name || 'Chair');
